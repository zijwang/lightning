...
class RichProgressBar(ProgressBarBase):
    ...
    def refresh(self) -> None:
        if self.progress:
            ...
            print(f"<- {inspect.stack()[1][3]}\t<-\t{inspect.stack()[2][3]}")
    ...
    def on_sanity_check_end(self, trainer, pl_module):
        if self.progress is not None:
            self.progress.update(self.val_sanity_progress_bar_id, advance=0, visible=False, refresh=True)
    ...
    def _update(self, progress_bar_id: int, current: int, visible: bool = True) -> None:
        if self.progress is not None and self.is_enabled:
            ...
            self.progress.update(progress_bar_id, advance=advance, visible=visible, refresh=True)
    ...
    def on_test_batch_start(
        ...
        if self.test_progress_bar_id is not None:
            self.progress.update(self.test_progress_bar_id, advance=0, visible=False, refresh=True)
    ...
    def on_predict_batch_start(
        ...
        if self.predict_progress_bar_id is not None:
            self.progress.update(self.predict_progress_bar_id, advance=0, visible=False, refresh=True)
